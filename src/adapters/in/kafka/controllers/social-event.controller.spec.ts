...
describe('GameEventConsumer', () => {
  ...
  beforeEach(async () => {
    ...
    socialEventController = module.get<SocialEventController>(SocialEventController);
  ...
  it(`should fail to call a use-case when there is no processor that supports the received events`, async () => {
    ...
    await socialEventController.handleNewFollowerTopicMessage(socialNewFollowerEventMock);
  ...
  it(`should call to use-case with received events from "${SOCIAL_NEW_FOLLOWER_TOPIC}" topic`, async () => {
    ...
    expect(useCases[0].processEvent).toHaveBeenCalledWith(socialNewFollowerEventMock);
  });

  it(`should call to use-case with received events from "${SOCIAL_FRIEND_REQUEST_TOPIC}" topic`, async () => {

    jest.spyOn(useCases[0], 'processEvent');
    await socialEventController.handleFriendRequestTopicMessage(socialFriendRequestEventMock);

    expect(useCases[0].processEvent).toHaveBeenCalledTimes(1);
    expect(useCases[0].processEvent).toHaveBeenCalledWith(socialFriendRequestEventMock);
  ...
  it(`should call to use-case with received events from "${SOCIAL_FRIEND_REQUEST_ACCEPTED_TOPIC}" topic`, async () => {
    ...
    jest.spyOn(useCases[0], 'processEvent');
    await socialEventController.handleFriendRequestAcceptedTopicMessage(socialFriendRequestAcceptedEventEventMock);

    expect(useCases[0].processEvent).toHaveBeenCalledTimes(1);
    expect(useCases[0].processEvent).toHaveBeenCalledWith(socialFriendRequestAcceptedEventEventMock);
